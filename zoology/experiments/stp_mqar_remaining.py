...
sweep_name = "stp-mqar-v2r-" + sweep_id
VOCAB_SIZE = 8_192
...
for d_model in [64, 128]:
    models.append(make_model(d_model, dict(name="zoology.mixers.based.Based", kwargs={"l_max": input_seq_len, "feature_dim": 16, "feature_name": "taylor_exp", "num_key_value_heads": 1, "num_heads": 1, "train_view": "quadratic"}), "based"))
    models.append(make_model(d_model, dict(name="zoology.mixers.delta_net.DeltaNet", kwargs={"l_max": input_seq_len, "num_heads": 2, "use_beta": True, "use_gate": False, "use_short_conv": True, "conv_size": 4}), "delta_net"))
    models.append(make_model(d_model, dict(name="zoology.mixers.gla.GatedLinearAttention", kwargs={"num_heads": 2, "use_short_conv": False}), "gla"))
...
print(f"  Models: based, delta_net, gla, stp_attn_A, hybrid_stp_B")
print(f"  Skipped: softmax_attn (already completed)")
